    ...
    <section className="section-glow relative overflow-hidden bg-[#060b1d] py-24 sm:py-32">
      ...
      <div className="section-shell relative z-10">
        <Reveal className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="font-display text-4xl leading-tight font-semibold tracking-[-0.04em] text-white sm:text-5xl">Organizador y aliados estratégicos</h2>
          <p className="mt-5 leading-7 text-slate-300">El INSNSB articula esta iniciativa con la SGTD-PCM, PUCP, ESAN y aliados del ecosistema de innovación.</p>
        </Reveal>
      </div>
        ...
        <div className="overflow-hidden" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
          <motion.div className="flex w-max gap-4 px-2" style={{ x }}>
            {[...partners, ...partners].map((partner, index) => {
              ...
              return (
                <article key={`${partner.name}-${index}`} className="group flex h-28 w-60 shrink-0 items-center gap-4 rounded-[26px] border border-white/10 bg-slate-900/75 px-5 shadow-[0_18px_45px_rgba(0,0,0,0.25)] backdrop-blur-xl transition-all hover:-translate-y-1 hover:border-violet-400/30">
                  <span className={`flex h-12 shrink-0 items-center justify-center rounded-2xl bg-white p-2 text-indigo-600 transition-transform group-hover:scale-105 ${partner.logo ? 'w-24' : 'w-16'}`}>
                    {partner.logo ? <img src={partner.logo} alt={`Logo de ${partner.name}`} className="h-full w-full object-contain" /> : Icon && <Icon className="h-6 w-6" />}
                  </span>
                  <span><span className="block font-display text-sm font-semibold text-white">{partner.name}</span><span className="mt-1 block text-xs text-slate-400">{partner.type}</span></span>
                </article>
              )
            })}
          </motion.div>
